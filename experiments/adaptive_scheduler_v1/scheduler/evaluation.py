...
class StaticPolicy:
    """Fixed-action baseline using the same online evaluation path."""

    frozen = True

    def __init__(self, action: Action):
        self.action = action
        self.algorithm_id = f"Static-{action.value}"

    def select_action(self, encoded_state, *, training: bool = False) -> Action:
        return self.action


def evaluate_online_episode(
    policy: TabularQPolicy | DQNPolicy | CFSMScheduler | StaticPolicy,
    episode: NpzTraceEpisode,
    *,
