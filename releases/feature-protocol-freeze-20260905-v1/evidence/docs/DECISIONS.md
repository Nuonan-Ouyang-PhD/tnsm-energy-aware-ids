...
23. FEATURE/LABEL PROTOCOL FREEZE PROPOSED as
    ...
    Rev 3 (2026-09-06, post-install verification-path correction),
    per user independent review of
    FEATURE_PROTOCOL_FREEZE_PROPOSAL_EVIDENCE_V1R2.zip (SHA-256
    `6319b1be2fd65f73d81121f02687c127315e89653399d031a7673ae4a982b77c`,
    3,263,669 bytes, 57 entries = 39 files + 18 directories),
    delivered with review package
    FEATURE_PROTOCOL_23_REV2_INDEPENDENT_REVIEW.zip (SHA-256
    `b5b4bdeb9e57951f4090e9ab9d07956d820d02b9291723f319ed54dd0e1dd11e`).
    The independent review passed package integrity, 79/79 packaged
    tests, the complete target and every spec binding, MI_dir
    preservation, zero admission, and deterministic replay. It found
    one remaining F23-02 execution defect: the Rev 2 procedure invoked
    the old-baseline replay verifier after the live path had already
    been replaced by the target, causing the generator's anchors to
    fail on an already-converted file.

    Rev 3 changes ONLY the verification phase/input boundary:

    (a) scripts/audits/verify_feature_policy_target_v1r2.py remains
        byte-identical (SHA-256
        `53c0b9233b11131a983b30506766877849936efea25f1b0b0ffa0f2f0d5562ef`)
        and is now explicitly PRE-INSTALL only. It consumes the old
        live baseline, deterministically rebuilds the target in
        memory, and retains its accepted 30-check diff/log evidence.

    (b) scripts/audits/verify_installed_feature_policy_v1r2.py is
        added as the POST-INSTALL read-only verifier (SHA-256
        `f6e9431f571d56cdbcdfe1fbd6846c0b8d7c60e9f7dfe3550af1ffd39ea6100e`).
        It receives five explicit inputs: a separately preserved old
        baseline, immutable target artifact, actual installed file,
        live label ontology, and target specification. It compares
        installed bytes directly to the pinned target, then checks the
        resulting state, bindings, evidence, zero admission, and gate
        authorization requirements without invoking the generator or
        writing any input. Exit 0 requires all 36 checks green and is
        explicitly NOT an execution authorization.

    (c) tests/test_feature_policy_postinstall_verifier.py invokes the
        real post-install CLI on distinct temporary paths. It covers
        the correct installed target plus seven nonzero-exit cases:
        old baseline not installed; changed admission count; changed
        MI_dir original reference; changed ontology; target and
        applied changed together; changed preserved baseline; and a
        missing gate authorization precondition. Each case asserts
        that all five inputs remain byte-identical during verification.
        The complete in-package suite passes 88/88 tests with no skips.

    (d) artifacts/proposals/feature_policy_freeze_target_spec_v1r2.json
        and FEATURE_MAPPING_PROTOCOL.md section 6.3 now give the exact
        ordered pre-install/install/post-install procedure and forbid
        rerunning the baseline replay against the installed target.

    Rev 3 does NOT execute #24. The target artifact remains
    byte-identical at
    `e81a55c16f9439b0356295353e8b342de19d5c5606d868952592f913beda714b`;
    config/feature_policy.json and config/label_ontology.json remain
    byte-identical at `2a903a4a...21b47` and `8a055e2e...6fab`.
    No proto census, mapping decision, label decision, admission set,
    or gate target is changed. Materialization, splitting, and
    training remain forbidden; #24 remains unauthorized.

24. FEATURE/LABEL PROTOCOL FROZEN as
    FEATURE-POLICY-20260905-V1-FROZEN. On the user's explicit limited
    authorization following independent acceptance of #23 Rev 3, the
    reviewed complete target bytes were installed verbatim at
    `config/feature_policy.json`. This authorization covered only the
    configuration freeze, verification, freeze record, and evidence
    packaging. It did NOT authorize materialization, splitting, or
    training.

    Authorization provenance: approved proposal package
    FEATURE_PROTOCOL_FREEZE_PROPOSAL_EVIDENCE_V1R3.zip, SHA-256
    `313e03a0fe702c11bc028adf16349d6aacfd211cf95b11aea5192b576e6a1d39`,
    3,271,836 bytes; independent review package
    FEATURE_PROTOCOL_23_REV3_INDEPENDENT_REVIEW.zip, SHA-256
    `806cf43ccb9c6f0781c16cc6be5a645c9e51de112ccf9a09c83de1c24e148cb0`;
    imported proposal source commit
    `c33113e7ca6a7d6edd9bf0a50d809ecadfb39a66`; freeze application
    commit `1989a9eb91dcc62ac77cf9f7bca3908b77050da0`.

    Execution time: 2026-09-05T23:58:15Z
    (2026-09-06T09:58:15+10:00 Australia/Melbourne).

    PRE-INSTALL: the live feature-policy hash was
    `2a903a4a0dd54e4307b7dce24599c39ce62b378b4cd8a95f8a633ffedb321b47`
    and the live ontology hash was
    `8a055e2e34bc8d70f62909f52441915589c310659d6ce3e427a2704820926fab`.
    The exact old feature-policy bytes were preserved separately as
    `artifacts/freezes/feature_policy_pre_freeze_baseline.json` with
    the same `2a903a4a...21b47` hash. The unchanged approved pre-install
    verifier ran with the available bundled Python 3.12.14 and passed
    30/30, exit 0. Its existing diff/log hashes remained
    `8d52dc00...77e60` and `ef300f2a...9cf84`. The first attempt under
    Apple Python 3.9.6 encountered the unsupported
    `Path.write_text(newline=...)` keyword only when writing those
    artifacts; it changed neither the live baseline nor the already
    byte-identical evidence. The accepted run used the unmodified
    verifier at SHA-256 `53c0b923...562ef`.

    INSTALLATION: the only installed source was
    `artifacts/proposals/feature_policy_freeze_target_v1r2.json`,
    SHA-256
    `e81a55c16f9439b0356295353e8b342de19d5c5606d868952592f913beda714b`.
    The installed `config/feature_policy.json` is byte-identical to
    that target and has the same hash. The target artifact itself was
    not regenerated or modified; `config/label_ontology.json` stayed
    byte-identical.

    POST-INSTALL: the five-input read-only verifier passed 36/36,
    exit 0, with the preserved baseline, immutable target, actual
    installed path, live ontology, and accepted spec kept distinct.
    It confirmed exactly four structured statuses frozen and zero
    proposed; semantic dispositions unchanged; MI_dir evidence element
    0 and resolution preserved; admitted_mapping_count = 0; the
    three-way and all pairwise cores EMPTY; every evidence/spec binding
    exact; and every input byte-identical before and after verification.
    Evidence is recorded in
    `artifacts/freezes/feature_policy_freeze_preinstall_v1.json`,
    `feature_policy_freeze_postinstall_v1.json`, and
    `feature_policy_freeze_execution_v1.json`.

    AUTHORIZATION BOUNDARY: the successful post-install report states
    `authorization_granted = false` and `data_gates_unlocked = false`.
    Each data-handling object still records `current` as "forbidden
    before protocol freeze" and retains the independent precondition
    "separate explicit user authorization for THIS gate obtained and
    recorded in DECISIONS.md". No such separate authorization was
    given in #24. Therefore materialization, splitting, and training
    remain forbidden and were not run. Stop after evidence packaging
    and await separate review/authorization.
