...
### 6.3 Rev 3 staging addendum (2026-09-06, verification-phase correction)

The independent review of the Rev 2 evidence package confirmed the
archive, all 79 packaged tests, complete target bytes, spec bindings,
MI_dir preservation, zero admissions, and deterministic pre-install
replay. It found one execution defect only: section 6.2 described
scripts/audits/verify_feature_policy_target_v1r2.py as if it could be
run after installation. That script intentionally rebuilds the target
from the old baseline at config/feature_policy.json; after installation
the path contains the target, so a second conversion fails. The target
configuration remains byte-identical at SHA-256 `e81a55c1...714b`.

Rev 3 separates the phases and inputs:

1. PRE-INSTALL, while config/feature_policy.json still has baseline
   SHA-256 `2a903a4a...21b47`, preserve those exact old bytes at a
   distinct read-only path and run
   scripts/audits/verify_feature_policy_target_v1r2.py. This remains
   the deterministic baseline-to-target replay and must pass 30
   checks. It must never be run against the installed target.
2. Only under separate explicit #24 authorization, install the fixed
   target bytes verbatim and re-hash the actual installed path to
   `e81a55c1...714b`.
3. POST-INSTALL, run the new read-only entry point against five
   explicit inputs:

   ```sh
   python -B scripts/audits/verify_installed_feature_policy_v1r2.py \
     --baseline /absolute/path/to/preserved-pre-24-feature_policy.json \
     --target artifacts/proposals/feature_policy_freeze_target_v1r2.json \
     --applied config/feature_policy.json \
     --label-ontology config/label_ontology.json \
     --spec artifacts/proposals/feature_policy_freeze_target_spec_v1r2.json
   ```

The post-install verifier (SHA-256
`f6e9431f571d56cdbcdfe1fbd6846c0b8d7c60e9f7dfe3550af1ffd39ea6100e`)
does not invoke the generator and writes JSON to stdout only. Its 36
checks pin the old baseline, immutable target, installed bytes, frozen
ontology, structured status census, dispositions, evidence bindings,
zero admissions, and all three still-authorization-gated operations.
The CLI tests use distinct temporary paths for a correct installation
and seven negative cases and assert that every input stays unchanged.
The complete in-package suite passes 88/88 tests with no skips.

This revision changes verification code, execution instructions, guard
tests, and consequent hashes only. Both live configs, every census
artifact, and all accepted semantics remain unchanged. #24 is NOT
authorized, and materialization, splitting, and training remain
forbidden.

## 7. #24 applied feature/label protocol freeze

The user authorized the limited #24 configuration freeze after the
independent PASS of #23 Rev 3. At 2026-09-05T23:58:15Z
(2026-09-06T09:58:15+10:00 Australia/Melbourne), the complete reviewed
target `artifacts/proposals/feature_policy_freeze_target_v1r2.json`
(SHA-256 `e81a55c1...714b`) was installed verbatim as
`config/feature_policy.json`. The effective policy is now frozen under
`FEATURE-POLICY-20260905-V1-FROZEN` and is byte-identical to the target.
The label ontology remains byte-identical at `8a055e2e...6fab`.

The old policy bytes are retained separately at
`artifacts/freezes/feature_policy_pre_freeze_baseline.json`
(`2a903a4a...21b47`). The unmodified pre-install replay passed 30/30
against that old state. The distinct post-install verifier then passed
36/36 against the preserved baseline, immutable target, actual installed
path, ontology, and spec; all five inputs remained unchanged. The full
records are under `artifacts/freezes/` and DECISIONS.md #24.

The frozen decision admits zero cross-dataset mappings: the three-way
core and all pairwise cores remain EMPTY. It preserves unresolved,
rejected, and derived semantic dispositions exactly as reviewed;
`frozen` locks those decisions and never converts them into admissions.

This #24 authorization did not authorize any data operation. The
materialization, splitting, and training objects still require separate
explicit per-gate user authorization recorded in DECISIONS.md. The
post-install report therefore records `authorization_granted = false`
and `data_gates_unlocked = false`; none of those operations was run.
