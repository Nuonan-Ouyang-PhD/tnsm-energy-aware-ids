...
Proposal-stage invariants asserted here:
  - the four structured status fields in the preserved pre-freeze snapshot
...
Rev 2 invariants additionally asserted here (F23-02 A/B/C):
  - the committed target artifact is a complete would-be-effective
    ...
    baseline by the archived generator script; after #24 these historical
    baseline checks run against the preserved pre-freeze snapshot;
...
Rev 3 invariants additionally asserted here:
  - pre-install deterministic replay and post-install installed-path
    verification are separate named phases with distinct inputs;
  - the post-install verifier is read-only, does not invoke the
    generator, does not grant authorization, and expects 36 checks;
  - before #24, the complete target and both live configs remained
    byte-identical to their reviewed states.

#24 freeze invariants are asserted separately in
tests/test_feature_policy_freeze_execution.py. Proposal-stage checks below
read the preserved old baseline where their historical meaning requires it.
"""
...
PRE_FREEZE_FEATURE_POLICY_PATH = (
    REPO_ROOT
    / "artifacts"
    / "freezes"
...
SPEC_SHA = (
    "ea97bc681fa6cc1125d02aa653d24c3fd14690c3700e1c65767799df2f63c232"
)
TARGET_GENERATOR = (
...
class FeaturePolicyProposalStateTests(unittest.TestCase):
    """The preserved #23 state has exactly four proposed fields."""

    def setUp(self):
        with PRE_FREEZE_FEATURE_POLICY_PATH.open(encoding="utf-8") as handle:
...
class ProposalConfigInvarianceTests(unittest.TestCase):
    """The preserved proposal config and ontology retain their SHAs."""

    def test_feature_policy_byte_identical(self):
        self.assertEqual(
            sha256_file(PRE_FREEZE_FEATURE_POLICY_PATH), FEATURE_POLICY_SHA
        )
...
class Rev2TargetArtifactTests(unittest.TestCase):
    ...
    configuration exists and reproduces from the preserved old baseline
    via the archived generator."""
    ...
    def test_target_is_a_complete_feature_policy(self):
        baseline = json.loads(
            PRE_FREEZE_FEATURE_POLICY_PATH.read_text(encoding="utf-8")
        )
        for key in baseline:
    ...
    def test_generator_rebuild_is_byte_identical(self):
        ...
        baseline_text = PRE_FREEZE_FEATURE_POLICY_PATH.read_text(
            encoding="utf-8")
    ...
    def test_preserved_baseline_and_installed_target_are_both_pinned(self):
        self.assertEqual(
            sha256_file(PRE_FREEZE_FEATURE_POLICY_PATH), FEATURE_POLICY_SHA
        )
        self.assertEqual(
            sha256_file(FEATURE_POLICY_PATH), TARGET_ARTIFACT_SHA
        )
        self.assertEqual(
            FEATURE_POLICY_PATH.read_bytes(), TARGET_ARTIFACT_PATH.read_bytes()
        )
...
class Rev2TargetSpecificationTests(unittest.TestCase):
    """The updated specification preserves bindings and splits phases."""
    ...
    def test_spec_sha_and_state(self):
        ...
        self.assertEqual(
            self.spec["decision_record"], "DECISIONS.md #23 Rev 3")

    def test_preinstall_and_postinstall_phases_are_distinct(self):
        state = self.spec["target_config_state"]
        pre = state["pre_install_verification"]
        post = state["post_install_verification"]
        self.assertEqual(
            pre["script"],
            "scripts/audits/verify_feature_policy_target_v1r2.py",
        )
        self.assertEqual(pre["expected_checks_total"], 30)
        self.assertTrue(pre["must_not_run_after_installation"])
        self.assertEqual(
            post["script"],
            "scripts/audits/verify_installed_feature_policy_v1r2.py",
        )
        self.assertEqual(post["expected_checks_total"], 36)
        self.assertEqual(
            set(post["arguments"]),
            {
                "--baseline",
        ...
        self.assertTrue(post["read_only"])
        self.assertFalse(post["invokes_generator"])
        self.assertFalse(post["grants_authorization"])
        self.assertFalse(post["unlocks_data_gates"])
        self.assertIn("PRE-INSTALL", state["application"])
        self.assertIn("POST-INSTALL", state["application"])
    ...
    def test_outcome_evidence_recorded(self):
        ...
        self.assertEqual(oe["verification_phase"], "pre_install")
        self.assertEqual(oe["post_install_checks_total"], 36)
        self.assertEqual(oe["post_install_cli_test_cases"], 8)
        self.assertEqual(oe["packaged_unittest_total"], 88)
        self.assertIn(
            "verify_installed_feature_policy_v1r2.py",
            oe["post_install_verification_script"],
        )
