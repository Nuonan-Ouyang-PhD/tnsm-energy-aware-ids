...
class RootFreezeProposalInputStateTests(unittest.TestCase):
    ...
    def test_feature_policy_was_still_proposed_after_22(self):
        with PRE_FREEZE_FEATURE_POLICY_PATH.open(encoding="utf-8") as handle:
            policy = json.load(handle)
        self.assertEqual(policy["status"], "proposed")

    def test_data_handling_ban_was_unchanged_after_22(self):
        with PRE_FREEZE_FEATURE_POLICY_PATH.open(encoding="utf-8") as handle:
            policy = json.load(handle)
...
class RootFreezeRecordTests(unittest.TestCase):
    ...
    must match the frozen bytes; the preserved feature-policy baseline
    retains the historical #22 out-of-scope state after #24."""
    ...
    def test_preserved_feature_policy_proves_untouched_by_22(self):
        import hashlib

        digest = hashlib.sha256(
            PRE_FREEZE_FEATURE_POLICY_PATH.read_bytes()
        ).hexdigest()
