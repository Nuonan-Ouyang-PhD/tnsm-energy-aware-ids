...
\usepackage{amsmath,amssymb,amsthm,booktabs,array,tabularx,url,microtype,caption,graphicx}
\usepackage{algorithm,algorithmic}
\usepackage[hidelinks]{hyperref}
\usepackage{fancyhdr}
\pagestyle{fancy}\fancyhf{}
\fancyhead[L]{TNSM reconstruction: evidence supplement}
...
\begin{document}
\begin{center}
{\LARGE Static and Dynamic Evidence Supplement}\\[6pt]
{\large Supplement to the post-results reconstruction manuscript}\\[4pt]
Final evidence supplement --- primary/P0/P1 locked; reviewer controls complete
\end{center}

\section{Scope and provenance}
Tables S1--S18 retain the accepted static, primary, and P0/P1 evidence. Tables S19 onward add the prospectively frozen reviewer controls R1/R2 and post-hoc R4 diagnostic. R3 is retained as a predeclared protocol stop, not silently omitted. No classifier or frozen policy was retrained, and no primary result was overwritten.

The static-stage original large archive was not re-audited in this revision. Its retained independent-review artifacts remain the numerical source for Tables S1--S7; the current adaptive archive supplies the later sections. These evidence layers must not be conflated. The six-file executed-source reconciliation has since been closed by an archival-only V2 recovery package: all six historical source files match the runtime-bound SHA-256 values exactly, and the later source copies remain separately retained. The post-hoc P1 strengthening archive is now complete and is incorporated in the final sections below. The primary V1 campaign remains unchanged and retains its own evidence identity.
...
\bottomrule\end{tabular}
\end{table}
The timing boundary excludes network capture, feature extraction, preprocessing, and the scheduling policy. These batch times must not be relabeled as single-record or end-to-end IDS latency. The power study used one batch per second; that pacing differs from the tight-loop timing study. The archived parity reports are not new hardware tests executed for this writing draft.
...
\begin{table}[h!]
\centering\small
\caption*{\textbf{Table S7.} Fifteen accepted static segments. Duration and energy refer to the observed meter interval. Maximum temperature uses workload-window events only. Version identifies the retained execution/logging version, not a different classifier training.}
\begin{tabular}{rllrrrr}\toprule
Repeat & Profile & Version & Duration (s) & Energy (J) & Mean (W) & Max. ($^\circ$C)\\\midrule
...
The historical root manifest has 371 valid listed entries. Two nested manifests were omitted:
\begin{quote}\small
\path{raw/materialization/MANIFEST_SHA256.txt}\\
\path{raw/materialization/materialization_run_v2/MANIFEST_SHA256.txt}
\end{quote}
A complete historical content list contains 373 files excluding the root manifest itself. The retained \texttt{CONTENT\_SHA256\_373.txt} refers to that original archive, not this smaller manuscript package. This draft's own manifest is separate. The original archive is neither modified nor silently repackaged here.
...
\end{table}
\begin{figure}[h!]
\centering\includegraphics[width=.80\linewidth]{figures/model_occupancy.pdf}
...
\end{figure}
\begin{figure}[h!]
\centering\includegraphics[width=.80\linewidth]{figures/phase_recall.pdf}
...
Tabular-Q and DQN each use one policy-training seed, 20260907, with 1,000 software episodes of 500 windows. The ten training traces and five validation traces are source-ID disjoint from the ten test traces. Validation is recorded every 25 episodes. The frozen Tabular-Q checkpoint comes from episode 50 and the DQN checkpoint from episode 25 (one-based numbering), not automatically from the final episode. Full curves, checkpoint metadata, and all 324 reconstructed greedy state actions are retained. The original harness did not instrument training energy. Archived process-start timestamps (06:10:58 and 06:11:41 UTC) and policy-metadata creation timestamps (06:11:24.687905 and 06:15:57.922987 UTC) indicate approximately 26.7~s for the Tabular-Q run and 256.9~s (4.28~min) for DQN. These are provenance-derived wall-clock intervals rather than calibrated compute benchmarks.

\begin{figure}[h!]
\centering\includegraphics[width=.94\linewidth]{figures/training_returns.pdf}
...
\end{figure}
\begin{figure}[h!]
\centering\includegraphics[width=.94\linewidth]{figures/validation_returns.pdf}
...
\]
\begin{theorem}[Pairwise non-degeneracy]\label{thm:supp-nondeg}
For $w_D>0$, a strict preference reversal between actions $a$ and $b$ exists on support $\mathcal S$ if and only if
\[
\min_{s\in\mathcal S}\Delta D_{ab}(s)<\tau_{ab}<\max_{s\in\mathcal S}\Delta D_{ab}(s).
\]
If this condition fails, every state in $\mathcal S$ gives the same sign of $U_a(s)-U_b(s)$.
\end{theorem}

\begin{proof}
Because $w_D>0$, the sign of $U_a(s)-U_b(s)$ equals the sign of $\Delta D_{ab}(s)-\tau_{ab}$. A strict reversal requires states $s,s'\in\mathcal S$ with opposite signs, equivalently one detection difference below $\tau_{ab}$ and another above it. This is exactly the strict interval condition in Theorem~\ref{thm:supp-nondeg}.
\end{proof}

\begin{corollary}[Bounded detection]
If $D_a(s),D_b(s)\in[0,1]$, reversal is impossible whenever $|\tau_{ab}|\ge 1$, or whenever the attainable interval $[\min\Delta D_{ab},\max\Delta D_{ab}]$ does not straddle $\tau_{ab}$.
\end{corollary}

With multiple normalized resource terms, $\Delta C_{ab}=\sum_j w_j(\bar C_{j,a}-\bar C_{j,b})$. Candidate-pool min--max scaling therefore makes the implied security--resource exchange rate depend on pool membership; an external power, latency-SLO, energy, or thermal budget provides a stable deployment reference.

\begin{algorithm}[h!]
\caption{Validation-only reward audit}\label{alg:supp-audit}
\footnotesize
\begin{algorithmic}[1]
\REQUIRE Actions $\mathcal M$; declared reward weights; cost profile $\{C_a\}$; validation support $\mathcal S$; detection scores $D_a$
\ENSURE Pairwise reversal-feasibility matrix $R$
\FOR{each unordered pair $(a,b)$}
  \STATE $\tau_{ab}\leftarrow(C_a-C_b)/w_D$
  \STATE $\Delta_{\min}\leftarrow\min_{s\in\mathcal S}\Delta D_{ab}(s)$; $\Delta_{\max}\leftarrow\max_{s\in\mathcal S}\Delta D_{ab}(s)$
  \STATE $R_{ab}\leftarrow\text{attainable}$ iff $\Delta_{\min}<\tau_{ab}<\Delta_{\max}$
\ENDFOR
\STATE Flag a reward as degenerate on $\mathcal S$ if no policy-relevant pair is attainable.
\end{algorithmic}
\end{algorithm}

Algorithm~\ref{alg:supp-audit} uses only quantities available at policy freeze and therefore does not inspect test outcomes. A failed audit calls for an audited static schedule or a pre-test redesign of the normalization, not post-hoc reward tuning.

\begin{table}[h!]
\centering\small
\caption*{\textbf{Table S11A.} Pairwise thresholds $\tau_{ab}$ under the frozen candidate-range normalization. Each value is the balanced-accuracy improvement action $a$ must offer over $b$ to overtake it.}
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lrrrr}\toprule
$a\backslash b$ & TinyDT & LightLR & MedRF & HeavyMLP\\\midrule
TinyDT     & --       & $-0.003$ & $-0.797$ & $-0.397$\\
LightLR    & $0.003$  & --       & $-0.794$ & $-0.394$\\
MedRF      & $0.797$  & $0.794$  & --       & $0.399$\\
HeavyMLP   & $0.397$  & $0.394$  & $-0.399$ & --\\
\bottomrule\end{tabular}
\end{table}
...
The six runtime paths are \path{controller.py}, \path{worker.py}, \path{scheduler/artifacts.py}, \path{scheduler/cli.py}, \path{scheduler/evaluation.py}, and \path{scheduler/trace_io.py}. An archival-only V2 recovery package now contains all six historically executed byte sequences. Independent verification confirms 6/6 exact SHA-256 matches to the runtime-bound expected values, verifies all 17 content entries in its manifest, and retains the later source versions separately. No experiment was rerun and no V1 evidence was modified for this closure. The historical primary archive itself does not isolate scheduler decision overhead. That gap is addressed only by the separate P1B corrected microbenchmark described below; source recovery alone does not add measurement evidence.

\section{P1A matched Static-LightLR physical control}
P1A is a post-hoc attribution experiment designed after the primary learned policies were observed to select LightLR throughout. It contains ten matched blocks with Static-LightLR, Tabular-Q, DQN, and Static-MedRF. The archive retains 42 attempted runs: 40 valid formal runs, one DQN preparation failure before workload execution, and one Static-MedRF interruption after 403/500 power samples. The interrupted run is linked to exactly one authorized \texttt{retry1} replacement. No invalid attempt is silently deleted or combined with its replacement.
...
\caption*{\textbf{Table S14.} P1A paired effects. Differences are first method minus second method. Exact sign-flip $p$ is a secondary robustness diagnostic; Student-$t$ intervals are the primary uncertainty summary.}
...
\begin{table}[h!]
\centering\footnotesize
\caption*{\textbf{Table S15.} P1B corrected decision overhead. The CI is across 30 block means; p95/p99 are means of block-level quantiles.}
...
\begin{figure}[h!]
\centering\includegraphics[width=.80\linewidth]{figures/p1b_scheduler_overhead_observed_support.pdf}
\caption*{\textbf{Figure S6.} Corrected P1B state-to-action timing on observed state support. The vertical axis is logarithmic.}
\end{figure}

\clearpage
\section{P1C multi-seed frozen-policy robustness}
...
\begin{table}[h!]
\centering\footnotesize
\caption*{\textbf{Table S16.} P1C greedy global action maps and held-out occupancy. Counts cover all 324 encoded states.}
\setlength{\tabcolsep}{3.3pt}
\begin{tabular}{lrrrrrr}\toprule
...
P1E contains 35 valid Tabular-Q runs over the predeclared discount/learning-rate matrix. All 35 select LightLR throughout held-out support and are single-action. These results are sensitivity evidence only; none is used to choose a replacement primary hyperparameter after test inspection.

\begin{table}[h!]
\centering\footnotesize
\caption*{\textbf{Table S18.} P1E Tabular-Q hyperparameter sensitivity on held-out support.}
\begin{tabular}{lrrrr}\toprule
...
\clearpage
\section{R1 fixed-rate confidence cascade}
R1 contains 40 valid 500-second runs in ten paired blocks. Static-LightLR, the frozen Tabular-Q policy, Static-MedRF, and the confidence cascade use the same trace within block; every method has an immediately preceding idle segment. The cascade band is fixed at $[0.3,0.7]$ and ground truth is never used for escalation. It sends 1.3388\% of rows to MedRF.

\begin{table}[h!]
\centering\footnotesize
\caption*{\textbf{Table S19.} R1 method means. Adjusted power subtracts the immediately preceding idle.}
...
\begin{table}[h!]
\centering\footnotesize
\caption*{\textbf{Table S19a.} R1 confidence-cascade call-granularity diagnostic reconstructed from 5,000 frozen window logs.}
...
\begin{figure}[h!]
\centering\includegraphics[width=.95\textwidth]{figures/revision_drift_adjusted_power.pdf}
\caption*{\textbf{Figure S7.} R1/R2 preceding-idle-adjusted power. Small points are runs; diamonds and bars are method means and Student-$t$ 95\% intervals over runs. Pairwise inference uses within-block differences in the CSV, not these marginal intervals.}
\end{figure}

\clearpage
\section{R2 variable-load factorial and DeadlineGuard}
...
\begin{table}[h!]
\centering\footnotesize
\caption*{\textbf{Table S20.} R2 method results over ten blocks. Energy is mean$\pm$SD across physical runs; completion is seconds.}
...
\caption*{\textbf{Table S21.} R2 paired effects against Static-LightLR. Energy and adjusted-power intervals use ten within-block differences. F1 is percentage points; $p$ is the exact two-sided sign-flip diagnostic for energy.}
...
\begin{table}[h!]
\centering\scriptsize
\caption*{\textbf{Table S22.} R2 state/action and service evidence. Occupancy totals 5,000 windows/method. Switch counts include the implementation's initialization convention.}
...
\begin{figure}[h!]
\centering\includegraphics[width=.75\textwidth]{figures/r2_dqn_block_energy_differences.pdf}
\caption*{\textbf{Figure S8a.} R2 DQN-minus-Static-LightLR raw energy by paired block. Nine differences are between 1.8 and 9.6 J; Block 4 is a 392.0-J power/temperature excursion.}
\end{figure}

\begin{figure}[h!]
\centering\includegraphics[width=.95\textwidth]{figures/r2_energy_security.pdf}
\caption*{\textbf{Figure S8.} R2 raw energy distributions and security operating points. The block is the repeat unit.}
\end{figure}

\clearpage
\section{R3 stop and R4 TinyDT threshold diagnostic}
R3 required four original observed-support states for controller-only whole-device measurement. The frozen source exposes eight and contains no outcome-independent rule for selecting four. The campaign therefore stopped before measurement. No controller-only power or joule value is inferred from P1B timing.

R4A selects threshold 0.6467505097 solely on validation balanced accuracy with the highest-threshold tie rule, freezes its hash, and applies it once to test. R4B is not executed because an independent stable reference load is unavailable.
...
\begin{figure}[h!]
\centering\includegraphics[width=.95\textwidth]{figures/r1_energy_security_frontier.pdf}
\caption*{\textbf{Figure S9.} R1 energy--security frontier. Horizontal bars are paired-block 95\% CIs for run energy; the plotted F1 values are method means.}
\end{figure}

\begin{figure}[h!]
\centering\includegraphics[width=.95\textwidth]{figures/r4_threshold_shift.pdf}
\caption*{\textbf{Figure S10.} R4 TinyDT threshold diagnostic. The primary threshold remains 0.5; the validation-selected threshold is a post-hoc diagnostic only.}
\end{figure}
...
\section{Final package integrity}
The final P0/P1 strengthening ZIP has SHA-256:
\begin{center}\small\texttt{d4e1e96960f1279c79445c744a02be1033e288dad27f440b87753934a3f4b108}\end{center}
Its size is 141,322,551 bytes and it contains 1,918 ZIP entries. Its root manifest contains 1,917 entries and independently covers every content file except itself; CRC, manifest, nested-manifest, and final validator checks pass. The reviewer-revision evidence ZIP has SHA-256:
\begin{center}\small\path{845ced5c96f932ee8d3509e2effcb39870a2fb9995bad9a6fe524f6c60b270e4}\end{center}
It has 1,912 entries and a 1,911-file root manifest. Its final validator reports R0/R1/R2/R4 PASS, R3 as a predeclared-ambiguity stop, no policy retraining, no test-driven selection, and retained invalid/replacement linkage. These properties establish internal provenance, not external validity.
\clearpage
