...
def parse_physical_csv_line(raw: bytes, path: Path, physical_line: int) -> List[str]:
    try:
        # The frozen inventory reader uses utf-8-sig. Consume only a BOM at
        # the beginning of the file; never strip a BOM from a data cell.
        text = raw.decode("utf-8-sig" if physical_line == 1 else "utf-8")
...
def audit_event(event: str, **values: Any) -> None:
    """Execution evidence outside the deterministic output tree."""
    print(json.dumps({"event": event, **values}, ensure_ascii=False),
          file=sys.stderr, flush=True)


def validate_source_header(raw: bytes, path: Path,
                           source_item: Mapping[str, Any]) -> List[str]:
    if not raw.endswith(b"\n"):
        raise MaterializationError("source header is not LF-terminated: %s" % path)
    header = parse_physical_csv_line(raw, path, 1)
    if header != source_item["columns"]:
        raise MaterializationError("source header mismatch: %s" % path)
    # Exact recipe in the frozen inventory generator, not raw first-line bytes.
    digest = hashlib.sha256("\x1f".join(header).encode("utf-8")).hexdigest()
    if digest != source_item["header_sha256"]:
        raise MaterializationError("source header SHA-256 mismatch: %s" % path)
    return header
...
def validate_contract_shape(contract: Mapping[str, Any]) -> None:
    if contract.get("header_identity") != {
            "inventory_sha256_recipe": "sha256(U+001F.join(parsed_header_columns).encode(UTF-8))",
            "bom_handling": "consume one leading UTF-8 BOM only at physical line 1",
            "raw_bytes_identity": "full source SHA-256 remains mandatory including BOM and line endings"}:
        raise MaterializationError("header identity recipe changed")
...
def scan_source_file(root: Path, source_item: Mapping[str, Any],
    ...
    try:
        with path.open("rb") as handle:
            ...
            header = validate_source_header(header_raw, path, source_item)
    ...
    audit_event("source_preflight_pass", source=str(path), relative_path=relative_path,
                sha256=observed_hash, bytes=stat_result.st_size, row_count=row_count,
                header_sha256=source_item["header_sha256"], excluded_rows=excluded)
...
def _serialize_shard(contract: Mapping[str, Any], dataset_id: str,
    ...
    try:
        with source_path.open("rb") as source:
            ...
            validate_source_header(header_raw, source_path, source_item)
...
def _execute_authorized(contract_path: Path, source_roots: Mapping[str, Path],
    ...
    audit_event("preflight_start", datasets=list(required_dataset_ids))
    scans = preflight_sources(contract, documents, source_roots)
    audit_event("preflight_complete", files=sum(len(items) for items in scans.values()))
    exception_index = expected_exception_index(documents["cic_quality_exceptions"])
    ...
    try:
        ...
        for dataset_id in required_dataset_ids:
            ...
            for ordinal, source_item in enumerate(items, start=1):
                ...
                audit_event("shard_rendered", dataset=dataset_id, ordinal=ordinal,
                            rows=rendered.row_count, total_completed_shards=total_completed_shards,
                            cumulative_output_bytes=budget.written)
            ...
            if replay:
                for ordinal, source_item in enumerate(items, start=1):
                    ...
                    audit_event("shard_replay_pass", dataset=dataset_id, ordinal=ordinal,
                                features_sha256=replayed.features_sha256,
                                labels_sha256=replayed.labels_sha256, rows=replayed.row_count)
        ...
        audit_event("publication_complete", output=str(output), output_bytes=budget.written)
        return {
            "contract_id": contract["contract_id"],
